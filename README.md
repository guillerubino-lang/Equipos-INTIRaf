...
- **Miniatura circular de la foto** → cada tarjeta muestra a la izquierda una foto chica (la de portada si hay una marcada, si no la primera subida) — mismo estilo que una foto de perfil. Se carga sola solo para las tarjetas que están en pantalla (o por entrar), a medida que scrolleás, así no se descargan las 700 de una. Usa la miniatura liviana que genera Google Drive (unos KB), no la foto completa — para que no se sienta lento ni pese de más en el celular. Si el equipo no tiene fotos, el círculo queda gris con "sin foto".
